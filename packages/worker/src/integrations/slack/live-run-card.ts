...
export class LiveRunCardCoordinator {
  ...
  private eventsForRun(runId: string): Array<{ cursor: number; event: DOToCLIEvent }> {
    ...
    const start = parsed.findIndex((entry) => entry.event.type === "agent_run_started" && entry.event.run_id === runId);
    if (start < 0) return [];
    const end = parsed.findIndex((entry, index) => index > start && entry.event.type === "agent_run_started");
    return parsed.slice(start, end < 0 ? parsed.length : end).filter((entry) =>
      entry.event.type === "agent_run_started"
      || runIdForEvent(entry.event) === runId
      || entry.event.type === "status"
      || entry.event.type === "phase"
      || entry.event.type === "agent_event");
  }
...
function isLiveRunEvent(event: DOToCLIEvent): boolean {
  return ["agent_run_started", "phase", "status", "agent_event", "question_raised", "question_answered", "approval_requested", "plan_execution_started", "agent_response", "agent_run_completed", "agent_run_failed"].includes(event.type);
}
